...
function addUnproccessedItems () {
  dbModel.instance.dialcode_batch.find({ status: 0, $limit: 100 },
    { allow_filtering: true, raw: true }, function (error, batches) {
      if (error) {
  ...
  dbModel.instance.dialcode_batch.find({ status: 1, $limit: 100 },
    { allow_filtering: true, raw: true }, function (error, batches) {
      if (error) {
        LOG.error({ filename, 'error while getting data from db for scheduled process ': error })
      } else {
